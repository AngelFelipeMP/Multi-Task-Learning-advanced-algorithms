...
for name in specific_data.keys():
    ...
    # specific_data[name]['df'] = specific_data[name]['df'].groupby(['heads','layer','data'], sort=False).mean() #COMMENT: I must try max and mean
    specific_data[name]['df'] = specific_data[name]['df'].loc[specific_data[name]['df'].groupby(['heads','layer','data'])[specific_data[name]['metric']].idxmax()]
